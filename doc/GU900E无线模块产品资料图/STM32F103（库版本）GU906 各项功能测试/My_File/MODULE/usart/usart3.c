...
void USART3_IRQHandler(void)
{
	...
	{	 	 
		if(RX_SIZ < MAXRECV){
			buff = USART_ReceiveData(USART3);//记录接收到的值
			if(buff || RX_SIZ){
				RX_BUFF[RX_SIZ++]  = buff;
			}
		} 
...
void usart3_Send(char *buf,int len)
{
	...
	while(USART_GetFlagStatus(USART3, USART_FLAG_TC) == RESET);
	while(USART_GetFlagStatus(USART3, USART_FLAG_TXE)==RESET){//发送一个字符完成产生中断
		USART_ClearITPendingBit(USART3 ,USART_IT_TXE);//清发送中断
	}
#if (USART3_485_EN == 1)
	RS485_TX_EN=0;
#endif
