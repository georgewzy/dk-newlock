...
int transport_sendPacketBuffer(int sock, unsigned char* buf, int buflen)
{
	...
	ret = gprs_send_at(cmd, ">", 15, 16);
	if(ret != NULL)
	{
		timer_delay_1ms(20);
		usart_send_data(USART2, buf, buflen);
		USART_OUT(USART1, "BB=%d:", buflen);
...
//		usart_send_data(USART1, &mqtt_buff.pdata[mqtt_buff_cnt], size);
		mqtt_buff_cnt += size;
		mqtt_buff.len = mqtt_buff.len - size;
		rc = size;
	}
	else
	{
...
int mqtt_subscribe_topic(unsigned char* topic, int req_qos, unsigned short packetid)
{
	...
	while(!ret)
	{
		...
		if(timer_is_timeout_1ms(timer_mqtt_subscribe_topic_timeout, 1000*10) == 0)
		{		
			topicString.cstring = topic;
			memset(buf, 0, sizeof(buf));
			//subscribe resend dup = 1
...
int mqtt_publish_qos0(unsigned char* topic, unsigned char* payload, int payload_len)
{
	...
	if(len > 0 )
	{
		...
		{
			USART_OUT(USART1, "mqtt_publist_qos0_gprs_wakeup_error=%d\r\n", len);
		}
	}
	else
	{
		USART_OUT(USART1, "mqtt_publist_qos0_len=%d\r\n", len);
	}
		...
		else
		{
			...
			if(list_send_empty_status == 1)//链表不为空
			{
				...
				{	
					...
					else if(tmp1->msg.status == PUBREL)
					{
						if(timer_send_list_pubrel_1ms(tmp1->msg.msg_id%5, 1000*10) == 0)	//重新发送
						{
							USART_OUT(USART1, "timer_send_list_1ms_cc11=%d\r\n", tmp1->msg.msg_id);
							USART_OUT(USART1, "timer_send_list_1ms_cc22=%d\r\n", tmp1->msg.msg_id%5);
							
							memset(buf, 0, sizeof(buf));
							len = MQTTSerialize_pubrel(buf, buflen, 1, tmp1->msg.msg_id);	//publisher resend pubrel dup=1
							if(len > 0)
							{
								...
								if(rc != -1)
								{
									list_modify_status(list_send, tmp1->msg.msg_id, PUBREL);
									list_send_travese(list_send);			
